export const StackData = [
  {
    name: "VS Code",
    icon: "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/2048px-Visual_Studio_Code_1.35_icon.svg.png",
  },
  {
    name: "HTML",
    icon: "https://cdn1.iconfinder.com/data/icons/programing-development-7/24/html_html5_web_programing_developer-512.png",
  },
  {
    name: "CSS",
    icon: "https://cdn4.iconfinder.com/data/icons/social-media-logos-6/512/121-css3-512.png",
  },
  {
    name: "Bootstrap",
    icon: "https://brandeps.com/icon-download/B/Bootstrap-icon-vector-07.svg",
  },
  {
    name: "JavaScript",
    icon: "https://www.freepnglogos.com/uploads/javascript-png/javascript-logo-transparent-logo-javascript-images-3.png",
  },
  {
    name: "React JS",
    icon: "https://cdn4.iconfinder.com/data/icons/logos-3/600/React.js_logo-512.png",
  },
  {
    name: "GitHub",
    icon: "https://cdn3.iconfinder.com/data/icons/free-social-icons/67/github_circle_gray-512.png",
  },
  {
    name: "Firebase",
    icon: "https://cdn4.iconfinder.com/data/icons/google-i-o-2016/512/google_firebase-2-512.png",
  },
  {
    name: "Next JS",
    icon: "https://ui-lib.com/blog/wp-content/uploads/2021/12/nextjs-boilerplate-logo.png",
  },
  {
    name: "Tailwind CSS",
    icon: "https://hight.io/_nuxt/img/tailwind.3558838.png",
  },
  {
    name: "Node JS",
    icon: "https://cdn4.iconfinder.com/data/icons/logos-and-brands/512/233_Node_Js_logo-512.png",
  },
  {
    name: "WebRTC",
    icon: "https://images.purevpn-tools.com/public/images/webrtc-logo.png",
  },
  {
    name: "Socket.io",
    icon: "https://upload.wikimedia.org/wikipedia/commons/9/96/Socket-io.svg",
  },
  {
    name: "Draft JS",
    icon: "https://draftjs.org/img/draftjs-logo.svg",
  },
  {
    name: "Framer Motion",
    icon: "https://user-images.githubusercontent.com/38039349/60953119-d3c6f300-a2fc-11e9-9596-4978e5d52180.png",
  },

  {
    name: "Express JS",
    icon: "https://cdn4.iconfinder.com/data/icons/logos-and-brands/512/187_Js_logo_logos-512.png",
  },
  {
    name: "Mongo DB",
    icon: "https://smartmindsteam.com/wp-content/uploads/2021/07/mon.png",
  },
  {
    name: "Sanity CMS",
    icon: "https://avatars.githubusercontent.com/u/17177659?s=280&v=4",
  },
  // {
  //   name: "",
  //   icon: "",
  // },
  // {
  //   name: "",
  //   icon: "",
  // },
  // {
  //   name: "",
  //   icon: "",
  // },
  // {
  //   name: "",
  //   icon: "",
  // },
];
